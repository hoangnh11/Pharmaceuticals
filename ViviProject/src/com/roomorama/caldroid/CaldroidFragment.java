...
public class CaldroidFragment extends DialogFragment {
	 ...
	 * Weekday conventions
	 */
	public static int SUNDAY = 1, MONDAY = 2, TUESDAY = 3, WEDNESDAY = 4,
	 ...
	 * Initial params key
	 */
	public final static String DIALOG_TITLE = "dialogTitle", MONTH = "month",
			YEAR = "year", SHOW_NAVIGATION_ARROWS = "showNavigationArrows",
			DISABLE_DATES = "disableDates", SELECTED_DATES = "selectedDates",
			MIN_DATE = "minDate", MAX_DATE = "maxDate",
			ENABLE_SWIPE = "enableSwipe", START_DAY_OF_WEEK = "startDayOfWeek",
	 ...
	 * however in many cases with compact screen, it can be collapsed to save
	 * space
	 */
	...
	/**
	 * Get current saved sates of the Caldroid. Useful for handling rotation. It
	 * does not need to save state of SQUARE_TEXT_VIEW_CELL because this may
	 * change on orientation change
	 */
	...
	protected void retrieveInitialArgs() {
		...
		if (args != null) {
			...
			// Get squareTextViewCell, by default, use square cell in portrait
			// mode
			// and using normal cell in landscape mode
			int orientation = getResources().getConfiguration().orientation;
			if (orientation == Configuration.ORIENTATION_PORTRAIT) {
				squareTextViewCell = args.getBoolean(SQUARE_TEXT_VIEW_CELL,
						true);
			} else {
				squareTextViewCell = args.getBoolean(SQUARE_TEXT_VIEW_CELL,
						false);
			...
			// Get theme
			themeResource = args
					.getInt(THEME_RESOURCE, R.style.CaldroidDefault);
		}
		if (month == -1 || year == -1) {
	...
	public static LayoutInflater getLayoutInflater(Context context,
			LayoutInflater origInflater, int themeResource) {
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		LayoutInflater localInflater = getLayoutInflater(getActivity(),
				inflater, themeResource);

		View view = localInflater.inflate(R.layout.calendar_view, container,
				false);
		...
		leftArrowButton.setBackgroundColor(getResources().getColor(
				R.color.Blue1));
		leftArrowButton.setImageResource(R.drawable.ic_arrow_left);

		rightArrowButton = (ImageView) view
				.findViewById(R.id.calendar_right_arrow);
		rightArrowButton.setBackgroundColor(getResources().getColor(
				R.color.Blue1));
		rightArrowButton.setImageResource(R.drawable.ic_arrow_right);
	...
	protected ArrayList<String> getDaysOfWeek() {
		String[] list = { "S", "M", "T", "W", "T", "F", "S" };
		ArrayList<String> ld = new ArrayList<String>();
		ld.addAll(Arrays.asList(list));
		return ld;
	}
	...
	public void onDismiss(DialogInterface dialog) {
		caldroidListener.onNothingSelected();
	}
}
