...
public class SearchListCustomerPending extends Activity implements OnClickListener{
	...
	private SearchListCustomerPendingAdapter searchListCustomerPendingAdapter;
	...
	public void onClick(View v) {
		...
		case R.id.linSearch:
			enStores = new EnArrayStores();
			searchListCustomerPendingAdapter = new SearchListCustomerPendingAdapter(SearchListCustomerPending.this, enStores.getStores());
			searchListCustomerPendingAdapter.setOnItemClickHandler(onItemClickHandler);					
			lvCustomer.setAdapter(searchListCustomerPendingAdapter);
			search = new Search("", "");
			search.execute();
			break;
	...
	OnClickListener onItemClickHandler = new OnClickListener() 
	{
        ...
        { 
            ...
            intent = new Intent(SearchListCustomerPending.this, CustomerDetails.class);
	...
	class Search extends AsyncTask<Void, Void, String> {
		...
		protected void onPreExecute() {
			progressDialog = new ProgressDialog(SearchListCustomerPending.this);
		...
		protected String doInBackground(Void... params) {
			if (!isCancelled()) {				
				...
				netParameter[0] = new NetParameter("access-token", BuManagement.getToken(SearchListCustomerPending.this));
		...
		protected void onPostExecute(String result) {
			...
			if (!isCancelled()) {
				if (result.equals(GlobalParams.TRUE) && enStores != null && enStores.getStores().size() > 0) {					
					...
					app.hideKeyboard(SearchListCustomerPending.this, edtFilter);
				}
			}
		}
	}
